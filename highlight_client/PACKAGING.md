...
注意：Tauri 更新包必须签名校验。私钥丢失后，已经安装的旧客户端将无法升级到后续版本，需要重新分发完整安装包。`v0.1.10` / `v0.1.11` 发布时没有生成完整有效的 `latest.json`，因此需要用户手动安装 `v0.1.12` 或更新版本；从配置了新私钥并同时发布 macOS `app` 更新包的版本开始，后续版本才能通过“检查更新”自动升级。
